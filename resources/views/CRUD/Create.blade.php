...
@section('content')
    ...
    <form action="{{ route('projects.store') }}" method="POST" enctype="multipart/form-data"
        ...
        <textarea name="description" id="description" placeholder="Project description"
            class="uppercase font-antonio font-bold text-center p-2 m-5 bg-icon placeholder:text-text border rounded-md"
            rows="4" required></textarea>

        <input type="file" name="image" id="image"
            class="uppercase font-antonio font-bold  text-center m-5 bg-icon p-2" required>

        <button type="submit" class="uppercase font-antonio font-bold text-center m-5 p-2 bg-icon border rounded-md">Create
            project</button>
    </form>
    <a href="/projects">
